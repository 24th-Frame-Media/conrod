...
#>
param(
  [string]$PythonRelease = (Join-Path $env:USERPROFILE 'Downloads\Conrod-0.2.11-win64\Conrod'),
...
$ocr = Join-Path $PythonRelease '_internal\rapidocr_onnxruntime\models'
...
$files = @(
  ...
  @('ch_PP-OCRv4_det_infer.onnx', "$ocr\ch_PP-OCRv4_det_infer.onnx", 'Apache-2.0 (PaddleOCR via RapidOCR)'),
  @('ch_PP-OCRv4_rec_infer.onnx', "$ocr\ch_PP-OCRv4_rec_infer.onnx", 'Apache-2.0 (PaddleOCR via RapidOCR)')
)
# Upstream URLs (from the Python code and libraries); the release mirror is tried first,
# and a download only counts if its SHA-256 equals the pin.
$upstream = @{
